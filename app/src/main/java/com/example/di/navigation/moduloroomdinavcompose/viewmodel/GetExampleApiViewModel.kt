...
class GetExampleApiViewModel @Inject constructor(
    savedStateHandle: SavedStateHandle,
)  : ViewModel() {
    //Referencia:  https://www.geeksforgeeks.org/retrofit-with-kotlin-coroutine-in-android/


    private val _apiResult= MutableStateFlow<AnimeList>(AnimeList())
    val apiResult = _apiResult.asStateFlow()

    //Aca va la implementacion del RetrofitHelper personalizado
    val animeApi = AnimeRetrofitHelper.getInstance().create(AnimeApi::class.java)


    fun loadData(){
        viewModelScope.launch(Dispatchers.IO) {

            val result: Response<AnimeList> = animeApi.getNaruto()
